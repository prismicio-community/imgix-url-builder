...
const generateUrlParamsTypes = async (): Promise<string> => {
	...
	sourceFile.addStatements("// Types generated using `imgix-url-params` as a source.")
	...
	sourceFile.addTypeAliases([
		...
		{
			name: "ColorKeyword",
			type: imgixParams.colorKeywordValues.map((value) => `"${value}"`).join(" | "),
			isExported: true,
		},
		{
			name: "Font",
	...
	for (const key in imgixParams.parameters) {
		const spec = imgixParams.parameters[key as keyof typeof imgixParams.parameters]
		...
		if ("default" in spec && baseDocs.tags) {
			baseDocs.tags.push({
				...
				text: typeof spec.default === "string" ? `\`"${spec.default}"\`` : `\`${spec.default}\``,
			})
		}
	...
	const output = sourceFile.print().replace(new RegExp(BLANK_LINE_IDENTIFIER, "g"), "")
