...
/**
 * Builds an `<img>` `srcset` attribute value for a given set of widths. It can also optinally apply
 * Imgix URL API parameters to the URLs. The URLs are signed by appending a signature to their URL
 * parameters. This locks the URLs and their parameters to the signature to prevent URL tampering.
 *
 * The `width` URL parameter will be applied for each `srcset` entry. If a `width` or `w` parameter
 * is provided to the `params` parameter, it will be ignored.
 *
 * **Important**: This function should only be used in a trusted environment and never in a browser.
