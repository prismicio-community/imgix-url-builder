...
export type SecureClientOptions = {
	/**
	 * The base URL used to construct image URLs from a path. The base URL must include the protocol,
	 * domain, and optionally a path.
	 *
	 * @example
	 * 	;`https://example.imgix.net`
	 *
	 * @example
	 * 	;`https://example.imgix.net/folder`
	 */
	baseURL: string
...
/**
 * An Imgix Rendering API client with automatic URL signing. A client is paired to a single Imgix
 * domain and secure URL token.
 *
 * **Important**: This client should only be used in a trusted environment and never in a browser.
...
export class SecureClient {
	...
	/**
	 * Builds a Web Proxy URL that proxies a non-Imgix URL to Imgix. The client's Imgix domain must be
	 * configured as a Web Proxy source. All Web Proxy URLs will be signed automatically.
	 *
	 * **Important**: The given URL must not be URI encoded. If it is, decode it before passing it to
	 * `buildWebProxyURL()`.
	 *
	 * @example
	 * 	;```ts
	...
	/**
	 * Builds a URL to an Imgix image with Imgix URL API parameters. The URL is signed by appending a
	 * signature to the URL parameters. This locks the URL and its parameters to the signature to
	 * prevent URL tampering.
	 *
	 * The given URL must be a full absolute URL containing the protocol and domain.
	 *
	 * URL parameters already applied to the image will be retained. To remove existing parameters,
	...
	/**
	 * Builds an `<img>` `srcset` attribute value for a given set of widths. It can also optinally
	 * apply Imgix URL API parameters to the URLs. The URLs are signed by appending a signature to
	 * their URL parameters. This locks the URLs and their parameters to the signature to prevent URL
	 * tampering.
	 *
	 * The `width` URL parameter will be applied for each `srcset` entry. If a `width` or `w`
	 ...
	 *
	 * @param url - Full absolute URL to the Imgix image.
	 * @param params - An object of Imgix URL API parameters. The `widths` parameter defines the
	 *   resulting `srcset` widths.
	...
	/**
	 * Builds an `<img>` `srcset` attribute value for a given set of widths for the client's base URL.
	 * It can also optinally apply Imgix URL API parameters to the URLs.
	 *
	 * The `width` URL parameter will be applied for each `srcset` entry. If a `width` or `w`
	 * parameter is provided to the `params` parameter, it will be ignored.
	 *
	 * Note: The returned URLs are not signed. See `buildSignedWidthSrcSet` if signatures are
	 * required.
	 *
	 * @example
	 * 	;```ts
	...
	/**
	 * Builds an `<img>` `srcset` attribute value for a given set of widths for the client's base URL.
	 * It can also optinally apply Imgix URL API parameters to the URLs. The URLs are signed by
	 * appending a signature to their URL parameters. This locks the URLs and their parameters to the
	...
	/**
	 * Builds an `<img>` `srcset` attribute value for a given set of pixel densities. It can also
	 * optinally apply Imgix URL API parameters to the URLs. The URLs are signed by appending a
	 * signature to their URL parameters. This locks the URLs and their parameters to the signature to
	 * prevent URL tampering.
	 *
	 * The `dpr` URL parameter will be applied for each `srcset` entry. If a `dpr` parameter is
	 ...
	 *
	 * @param url - Full absolute URL to the Imgix image.
	 * @param params - An object of Imgix URL API parameters. The `pixelDensities` parameter defines
	 *   the resulting `srcset` widths.
	 * @returns A `srcset` attribute value for `url` with the given Imgix URL API
	 * parameters applied.
	 */
	...
	/**
	 * Builds an `<img>` `srcset` attribute value for a given set of pixel densities for the client's
	 * base URL. It can also optinally apply Imgix URL API parameters to the URLs.
	 *
	 * The `dpr` URL parameter will be applied for each `srcset` entry. If a `dpr` parameter is
	 * provided to the `params` parameter, it will be ignored.
	 *
	 * Note: The returned URLs are not signed. See `buildSignedPixelDensitySrcSet` if signatures are
	 * required.
	 *
	 * @example
	 * 	;```ts
	...
	/**
	 * Builds an `<img>` `srcset` attribute value for a given set of pixel densities for the client's
	 * base URL. It can also optinally apply Imgix URL API parameters to the URLs. The URLs are signed
	 * by appending a signature to their URL parameters. This locks the URLs and their parameters to
	 * the signature to prevent URL tampering.
	 *
	 * The `dpr` URL parameter will be applied for each `srcset` entry. If a `dpr` parameter is
	 * provided to the `params` parameter, it will be ignored.
	 *
	 ...
	 * 		secureURLToken: "example-token",
	 * 	})
	 * 	const srcset = client.buildSignedPixelDensitySrcSetForPath("/image.png", {
	 * 		pixelDensities: [1, 2, 3],
	 * 	})
	 * 	// => https://example.imgix.net/image.png?dpr=1&s=def3e221c3f4c4debda091b8e49420ea 1x,
	 * 	//    https://example.imgix.net/image.png?dpr=2&s=f12c7c39333410c10c2930b57116a943 2x,
