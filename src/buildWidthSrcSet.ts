 ...
 * Imgix URL API parameters to the URLs.
 *
 * The `width` URL parameter will be applied for each `srcset` entry. If a `width` or `w` parameter
 * is provided to the `params` parameter, it will be ignored. Similarly, if a `height` or `h`
 * parameter is provided to the `params` parameter, it will be ignored to prevent unexpected aspect
 * ratio.
 *
 * @example
 * 	```ts
